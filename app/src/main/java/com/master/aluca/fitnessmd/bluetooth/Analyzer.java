...
public class Analyzer {
    ...
    private static final String LOG_TAG = "Fitness_Analyzer";
    ...
    public static ActivityReport analyzeAccel(ArrayList<ContentObject> objectArray, int samplingInterval, int totalTime) {
		Log.d(LOG_TAG, "analyzeAccel");

        if(objectArray == null || objectArray.size() < 1) {
			Log.d(LOG_TAG, "objectArray == null");
            return null;
        }
        ...
        int idx = co1.getAccelData().length/3;
        ...
        for(int i = 0; i < idx; ++i) {
            ...
            nX[i] = (float)(co1.getAccelData()[i*3]+32768)/65535.f;
			Log.d(LOG_TAG, "nX[" + i + "] : " + nX[i]);
            //Logs.d("#"+nX[i]);
            nY[i] = (float)(co1.getAccelData()[i*3+1]+32768)/65535.f;
			Log.d(LOG_TAG, "nY[" + i + "] : " + nY[i]);
            //Logs.d("#"+nY[i]);
            nZ[i] = (float)(co1.getAccelData()[i*3+2]+32768)/65535.f;
			Log.d(LOG_TAG, "nZ[" + i + "] : " + nZ[i]);
            //Logs.d("#"+nZ[i]);
            n3D[i] = (float) Math.sqrt(nX[i]*nX[i]+nY[i]*nY[i]+nZ[i]*nZ[i]);
			Log.d(LOG_TAG, "n3D[" + i + "] : " + n3D[i]);
            //Log.d(LOG_TAG, "#" + n3D[i]);
        }
        ...
        {
			Log.d(LOG_TAG, "res[" + i + "] : " + res[i]);
			Log.d(LOG_TAG, "res[" + (i+1) + "] : " + res[i+1]);
            fStepTime = (res[i+1] - res[i])*0.05f;
            fStepVelocity = 0.5f/fStepTime;
            fAvgVelocity += fStepVelocity;
			Log.d(LOG_TAG, "fStepTime : " + fStepTime);
			Log.d(LOG_TAG, "fStepVelocity : " + fStepVelocity);
			Log.d(LOG_TAG, "fAvgVelocity : " + fAvgVelocity);
        }
        fAvgVelocity /= res.length;
        fAvgVelocity *= 3.6f; // convert m/s to km/h
        nLastDetectedTime = res[res.length-1];
		Log.d(LOG_TAG, "fAvgVelocity : " + fAvgVelocity);
		Log.d(LOG_TAG, "nLastDetectedTime : " + nLastDetectedTime);

        nStepCount += res.length;
        ar.mShakeActionCount = nStepCount;

		Log.d(LOG_TAG, "nStepCount : " + nStepCount);
		Log.d(LOG_TAG, "ar.mShakeActionCount : " + ar.mShakeActionCount);

        Log.d(LOG_TAG,"#");
        Log.d(LOG_TAG,"# of Xdata: "+nX.length+", shake: "+ar.mShakeActionCount);
...
//			ar.mAverageDifference = 0;
    ...
    public static double calculateCalorie(int shakeActionCount) {
		Log.d(LOG_TAG, "calculateCalorie");
        return shakeActionCount*50;
    }

    public static void setWeight(double weight) {
		Log.d(LOG_TAG, "setWeight");
        // Set user weight
