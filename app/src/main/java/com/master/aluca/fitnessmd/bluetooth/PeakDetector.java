...
public class PeakDetector {
    ...
    private static final String LOG_TAG  ="Fitness_PeakDetector";
    ...
    public int[] process(final int windowSize, final float stringency) {
        Log.d(LOG_TAG,"process windowSize : " + windowSize + " >>> stringency : " + stringency);
        ...
        for (int i = windowSize; i < S.length - windowSize; i++) {
            Log.d(LOG_TAG,"T[ : " + (i-1) + "] : " + T[i-1]);
            Log.d(LOG_TAG,"T[ : " + i + "] : " + T[i]);
            Log.d(LOG_TAG,"T[ : " + (i+1) + "] : " + T[i+1]);
            maxLeft = Math.abs(T[i] - T[i-1]) > offset ? T[i] - T[i-1] : 0;
            maxRight = Math.abs(T[i] - T[i+1]) > offset ? T[i] - T[i+1] : 0;
            Log.d(LOG_TAG,"maxLeft : " + maxLeft);
            Log.d(LOG_TAG,"maxRight : " + maxRight);
        ...
        Log.d(LOG_TAG,"#");
        Log.d(LOG_TAG,"maxLeft: "+maxLeft+", maxRight: "+maxRight+", mean: "+ mean+", variance: " +variance+", std: " + std);
        ...
        for (int i = 0; i < peakLocations.size()-1; i++) {
            ...
            Log.d(LOG_TAG,"peak1 : " + peak1);
            Log.d(LOG_TAG,"peak2 : " + peak2);
        ...
        for (int i = 0; i < peakArray.length; i++) {
            ...
            Log.d(LOG_TAG,"peakArray[" + i + "] : " +  peakArray[i]);
        }
        return peakArray;
